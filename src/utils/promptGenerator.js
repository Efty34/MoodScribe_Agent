function generateMoviePrompt(stressEntryPercentage, pastLikings = null) {
  ...
  const calmingGenres = ["comedy", "drama", "animation", "romance", "documentary"];
  const engagingGenres = ["adventure", "sci-fi", "biography", "fantasy", "thriller"];
  ...
  const selectedCalmingThemes = shuffleArray(calmingThemes).slice(0, 3).join(", ");
  const selectedEngagingThemes = shuffleArray(engagingThemes).slice(0, 3).join(", ");
  const selectedCalmingGenres = shuffleArray(calmingGenres).slice(0, 3).join(", ");
  const selectedEngagingGenres = shuffleArray(engagingGenres).slice(0, 3).join(", ");
  ...
  if (pastLikings?.favoriteMovies?.length > 0) {
    prompt += `Take into account their favorite movies: ${JSON.stringify(pastLikings.favoriteMovies)}, but ensure variety and avoid repetition. `;
  }
...
function generateBookPrompt(stressEntryPercentage, pastLikings = null) {
  ...
  const calmingGenres = ["mindfulness", "light fiction", "self-help", "poetry", "nature writing"];
  const engagingGenres = ["adventure", "biography", "fantasy", "science fiction", "thriller"];
  ...
  if (pastLikings?.favoriteBooks?.length > 0) {
    prompt += `Take into account their favorite books: ${JSON.stringify(pastLikings.favoriteBooks)}, but ensure variety and avoid repetition. `;
  }
...
function generateMusicPrompt(stressEntryPercentage, pastLikings = null) {
  ...
  const calmingGenres = ["classical", "ambient", "acoustic", "lo-fi", "new age"];
  ...
  if (pastLikings?.favoriteSongs?.length > 0) {
    prompt += `Take into account their favorite songs: ${JSON.stringify(pastLikings.favoriteSongs)}, but ensure variety and avoid repetition. `;
  }
...
function generateExercisePrompt(stressEntryPercentage, pastLikings = null) {
  ...
  // Select random themes and genres based on stress level
  const selectedCalmingExercises = shuffleArray(calmingExerciseTypes).slice(0, 3);
  const selectedEngagingExercises = shuffleArray(engagingExerciseTypes).slice(0, 3);
  ...
  if (pastLikings?.favoriteExercises?.length > 0) {
    prompt += `Take into account their favorite exercises: ${JSON.stringify(pastLikings.favoriteExercises)}, but ensure variety and avoid repetition. `;
  }

  if (stressEntryPercentage >= 40) {
    prompt += `Since stress is significant, recommend calming yet effective exercises that focus on stress relief and mental well-being. Suggest exercises like ${selectedCalmingExercises.join(", ")}, with benefits such as ${selectedCalmingBenefits.join(", ")}.`;
  } else {
    prompt += `Since the mood is mostly positive, recommend engaging and energetic exercises that span multiple workout types. Suggest exercises like ${selectedEngagingExercises.join(", ")}, with benefits such as ${selectedEngagingBenefits.join(", ")}.`;
  }
